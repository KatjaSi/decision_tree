...
class DecisionTree:
    ...
    def fit(self, X, y):
        ...
        #node = Node()
        # If all examples are positive (or all negative), return the single 
        # Node tree root with positive/negative label
...
def traverse_tree(dt, tuples, matrix, visited = list()):
    """
    help function, realizes dfs search, is uses to generate the rules
    """
    if len(dt.children) == dt.visited_children:
        visited.append(dt)
    if dt.attribute is None:
        rule = list()
        rule.append(tuples)
...
def go_to_root(dt):
    """
        Traverses the pointer to the root of the tree
    """
